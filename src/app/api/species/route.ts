...
export async function GET() {
  try {
    const result = await sql`SELECT name FROM species`;
    const species: string[] = result.rows.map(row => row.name);
...
export async function POST(request: Request) {
  try {
    ...
    await sql`INSERT INTO species (name) VALUES (${species}) ON CONFLICT (name) DO NOTHING`;
    return NextResponse.json({ success: true });
  } catch (error) {
    console.error('Помилка збереження породи:', error);
    return NextResponse.json({ error: 'Не вдалося зберегти породу' }, { status: 500 });
  }
}